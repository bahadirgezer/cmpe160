...
public class Operations{
	 ...
	 */
	private static int sequentialContainer = 0;
	/**
	 ...
	 */
	static void createContainer(int portID, int weight, char type) {
		if (type == 'R') {
			Container container = new RefrigeratedContainer(sequentialContainer, weight);
			container.setPlaceID(portID);
			sequentialContainer+=1;
			containerList.add(container);
			portList.get(portID).getContainers().add(container);
		} else if (type == 'L') {
			Container container = new LiquidContainer(sequentialContainer, weight);
			container.setPlaceID(portID);
			sequentialContainer+=1;
			containerList.add(container);
			portList.get(portID).getContainers().add(container);
			...
			if (weight<=3000) {
				Container container = new BasicContainer(sequentialContainer, weight);
				container.setPlaceID(portID);
				sequentialContainer+=1;
				containerList.add(container);
				portList.get(portID).getContainers().add(container);
			} else {
				Container container = new HeavyContainer(sequentialContainer, weight);
				container.setPlaceID(portID);
				sequentialContainer+=1;
				containerList.add(container);
				portList.get(portID).getContainers().add(container);
	 ...
	 */
	private static int sequentialShip = 0;
	/**
	 ...
	 * It should be used to take the inputs from the RegularExpressions class.
	 * 
	 * @param portID the ID of the port where the ship will be placed initially.
	 ...
	 */
	static void createShip(int portID, int maxWeight,
			int maxAll, int maxHeavy, int maxRefrige, 
			int maxLiquid, double fuelCon) {
		Port port = portList.get(portID);
		Ship ship = new Ship(sequentialShip, port, maxWeight, maxAll, 
	 ...
	 */
	private static int sequentialPort = 0;
	/**
	...
	static void createPort(double X, double Y) {
		Port port = new Port(sequentialPort, X, Y);
		sequentialPort+=1;
		portList.add(port);
	}
...
//			output(System.out);
//			System.out.println("================================");
//			System.out.println("iteration: " + i+" input: "+ input);
